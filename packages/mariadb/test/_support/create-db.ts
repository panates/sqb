...
export async function createTestSchema(database: string) {
  ...
  try {
    ...
    const dataFiles = getInsertSQLsForTestData({ dialect: 'mariadb' });
    for (const table of dataFiles) {
      for (const script of table.scripts) {
        ...
        // quotes, which MariaDB (like MySQL) only accepts as identifiers
        // under ANSI_QUOTES sql_mode. Since @sqb/mariadb doesn't set that
        // mode, rewrite the one reserved identifier in this fixture data
