...
describe('connect:Repository.update()', () => {
  ...
  it('should update even element name and field name differs', async () => {
    ...
    expect(sql).toStrictEqual(
      'update customers T set given_name = $1 where T.id = $2',
    );
  ...
  it('should check enum value', async () => {
    ...
    await expect(() =>
      repo.update(1, { name: 'small', color: 'pink' }),
    ).rejects.toThrow('value must be one of');
  });

  it('should column is required', async () => {
    const repo = client.getRepository(Customer);
    await expect(() => repo.update(1, { countryCode: null })).rejects.toThrow(
      'is required',
    );
  });
});
